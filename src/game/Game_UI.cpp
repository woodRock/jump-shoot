...
void JumpShootGame::DrawButton(int x, int y, int w, int h,
  ...
  m_TextRenderer->RenderTextCentered(text, x + w / 2, y + h / 2 - 12,
                                     {255, 255, 255, 255});
}
...
void JumpShootGame::RenderMainMenu() {
  int w = m_Width;
  int h = m_Height;
  m_TextRenderer->RenderTextCentered("JUMP SHOOT", w / 2, 100,
                                     {255, 200, 50, 255});
  m_TextRenderer->RenderTextCentered("Parkour Archer", w / 2, 140,
                                     {200, 200, 200, 255});
  int btnW = 240;
  int btnH = 50;
  int startY = 300;
  ...
  if (!m_InOptions) {
    DrawButton(w / 2 - btnW / 2, startY, btnW, btnH, "PLAY",
               m_MenuSelection == 0);
    DrawButton(w / 2 - btnW / 2, startY + gap, btnW, btnH, "OPTIONS",
               m_MenuSelection == 1);
    DrawButton(w / 2 - btnW / 2, startY + gap * 2, btnW, btnH, "QUIT",
               m_MenuSelection == 2);
  } else {
    m_TextRenderer->RenderTextCentered("OPTIONS", w / 2, 240,
                                       {255, 255, 255, 255});
    bool isFullscreen =
        SDL_GetWindowFlags(m_Window) & SDL_WINDOW_FULLSCREEN_DESKTOP;
    std::string fsText = isFullscreen ? "FULLSCREEN: ON" : "FULLSCREEN: OFF";
    DrawButton(w / 2 - btnW / 2, startY, btnW, btnH, fsText,
               m_MenuSelection == 0);
...
void JumpShootGame::RenderPauseMenu() {
  ...
  SDL_RenderFillRect(m_Renderer, &rect);
  m_TextRenderer->RenderTextCentered("PAUSED", w / 2, 100,
                                     {255, 255, 255, 255});
  int btnW = 240;
  int btnH = 50;
  int startY = 250;
  ...
  if (!m_InOptions) {
    ...
    DrawButton(w / 2 - btnW / 2, startY + gap, btnW, btnH, "OPTIONS",
               m_MenuSelection == 1);
    DrawButton(w / 2 - btnW / 2, startY + gap * 2, btnW, btnH, "MAIN MENU",
               m_MenuSelection == 2);
    DrawButton(w / 2 - btnW / 2, startY + gap * 3, btnW, btnH, "QUIT",
               m_MenuSelection == 3);
  } else {
    m_TextRenderer->RenderTextCentered("OPTIONS", w / 2, 180,
                                       {255, 255, 255, 255});
    bool isFullscreen =
        SDL_GetWindowFlags(m_Window) & SDL_WINDOW_FULLSCREEN_DESKTOP;
    std::string fsText = isFullscreen ? "FULLSCREEN: ON" : "FULLSCREEN: OFF";
...
void JumpShootGame::RenderUI() {
  ...
  if (weapon) {
    ...
    if (m_ShakeTimer > 0) {
      bowX +=
          (int)(((rand() % 100) / 50.0f - 1.0f) * m_ShakeIntensity * 100.0f);
      bowY +=
          (int)(((rand() % 100) / 50.0f - 1.0f) * m_ShakeIntensity * 100.0f);
    }
    ...
    if (weapon->isDrawing) {
      ...
      SDL_SetRenderDrawColor(m_Renderer, (Uint8)(150 + 105 * power),
                             (Uint8)(255 * (1.0f - power)), 50, 255);
  ...
  if (m_HitmarkerTimer > 0) {
    SDL_SetRenderDrawColor(m_Renderer, 255, 255, 255, 255);
    int size = 10;
    SDL_RenderDrawLine(m_Renderer, w / 2 - size, h / 2 - size, w / 2 - size / 2,
                       h / 2 - size / 2);
    SDL_RenderDrawLine(m_Renderer, w / 2 + size, h / 2 - size, w / 2 + size / 2,
                       h / 2 - size / 2);
    SDL_RenderDrawLine(m_Renderer, w / 2 - size, h / 2 + size, w / 2 - size / 2,
  ...
  if (phys) {
    ...
    if (!phys->isGrounded) {
      SDL_Rect barBg = {20, h - 60, 100, 10};
      SDL_SetRenderDrawColor(m_Renderer, 50, 50, 50, 200);
      SDL_RenderFillRect(m_Renderer, &barBg);
      SDL_Rect barFg = {20, h - 60, (int)(phys->wallRunTimer * 50), 10};
      SDL_SetRenderDrawColor(m_Renderer, 50, 200, 255, 255);
      SDL_RenderFillRect(m_Renderer, &barFg);
    }
  }
  ...
  if (m_GameFinished) {
    ...
    m_TextRenderer->RenderTextCentered("COURSE COMPLETE!", w / 2, h / 2 - 40,
                                       {255, 200, 50, 255});
    char finalTime[64];
    snprintf(finalTime, 64, "FINAL TIME: %.3fs", m_RunTimer);
    m_TextRenderer->RenderTextCentered(finalTime, w / 2, h / 2 + 10,
                                       {255, 255, 255, 255});
    m_TextRenderer->RenderTextCentered("Press SPACE for Next Level", w / 2,
                                       h / 2 + 60, {200, 200, 200, 255});
    m_TextRenderer->RenderTextCentered("Press R to Retry", w / 2,
