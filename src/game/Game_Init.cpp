...
JumpShootGame::JumpShootGame()
    : Application("Jump Shoot - Parkour Archer", 800, 600) {}
...
JumpShootGame::~JumpShootGame() {
  ...
  if (m_SfxJump)
    Mix_FreeChunk(m_SfxJump);
  if (m_SfxGrapple)
    Mix_FreeChunk(m_SfxGrapple);
  if (m_Ambience)
    Mix_FreeMusic(m_Ambience);
}

void JumpShootGame::OnStart() {
  m_TextRenderer =
      std::make_unique<TextRenderer>(m_Renderer, "assets/font.ttf", 24);
  ...
  m_Ambience = Mix_LoadMUS("assets/ambience.mp3");
  if (!m_Ambience)
    m_Ambience = Mix_LoadMUS("assets/ambience.wav");
...
void JumpShootGame::InitGame() {
  ...
  // Player Setup (Same for all levels for now)
  ...
  if (m_CurrentLevel == 2) {
      ...
      m_Registry.AddComponent<Transform3DComponent>(t1, {19.5f, 3.5f, 2.0f, 0.0f, 0.0f});
